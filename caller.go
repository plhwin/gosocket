...
func (c *caller) callFunc(client interface{}, args interface{}, id string) []reflect.Value {
	...
	in := []reflect.Value{reflect.ValueOf(client), reflect.ValueOf(args).Elem(), reflect.ValueOf(id)}
	if !c.ArgsPresent {
		in = in[0:1]
	} else if !c.IdPresent {
		in = in[0:2]
